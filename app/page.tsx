...
const collections = [
  { name: "Pro Classic 8/32", tag: "Универсальная классика", price: "от 1 165 ₽/м²", image: "/images/collection/egger-laminate-pro-classic-8-32.jpg" },
  { name: "Pro Classic Aqua+", tag: "Водостойкий", price: "от 1 795 ₽/м²", image: "/images/collection/egger-laminate-classic-aqua-pl.jpg" },
  { name: "Pro Large 8/33", tag: "Широкая доска", price: "от 1 405 ₽/м²", image: "/images/collection/egger-laminate-pro-large-8-33-aqua-plus.jpg" },
  { name: "Pro Kingsize", tag: "Для просторных комнат", price: "от 1 268 ₽/м²", image: "/images/collection/egger-laminate-pro-kingsize-2v.jpg" },
];

const products = [
  { sku: "EPL179", name: "Дуб Сория натуральный", collection: "Pro Classic 10/33", price: 1630, old: 0, cls: "33", thickness: "10", water: "Влагостойкий", tone: "Натуральный", image: "/images/decor/egger-laminate-pro-classic-10-33-EPL179-dub-soriya-naturalnyj-NEW.jpg" },
  { sku: "EPL156", name: "Дуб Азгил медовый", collection: "Pro Classic 12/33", price: 1765, old: 0, cls: "33", thickness: "12", water: "Влагостойкий", tone: "Тёплый", image: "/images/decor/egger-laminate-pro-classic-12-33-EPL156-dub-azgil-medovyj.jpg" },
  { sku: "EPL185", name: "Дуб Шерман серый", collection: "Pro Large Aqua+", price: 1510, old: 0, cls: "32", thickness: "8", water: "Водостойкий", tone: "Серый", image: "/images/decor/egger-laminate-pro-large-8-32-aqua-plus-EPL185-dub-sherman-seryj.jpg" },
  { sku: "EPL255", name: "Клён Киото натуральный", collection: "Pro Classic 8/33", price: 1270, old: 0, cls: "33", thickness: "8", water: "Влагостойкий", tone: "Светлый", image: "/images/decor/egger-laminate-pro-classic-8-33-EPL255-klen-kioto-naturalnyj.jpg" },
  { sku: "EPL166", name: "Бетон Чикаго светло-серый", collection: "Pro Large Aqua+", price: 1940, old: 0, cls: "33", thickness: "8", water: "Водостойкий", tone: "Серый", image: "/images/decor/egger-laminate-pro-large-8-33-aqua-plus-EPL166-beton-chikago-svetlo-seryj.jpg" },
  { sku: "EPL172", name: "Сосна Инвери винтаж", collection: "Pro Classic 8/32", price: 1165, old: 0, cls: "32", thickness: "8", water: "Влагостойкий", tone: "Тёплый", image: "/images/decor/egger-laminate-pro-classic-8-32-EPL172-sosna-inveri-vintazh.jpg" },
];
...
function Assistant({onClose}:{onClose:()=>void}) {
  ...
  async function send(value?:string){ const message=(value??text).trim(); if(!message||loading)return; setMessages(m=>[...m,{role:"user",text:message}]); setText(""); setLoading(true);
    try { let res, data; for(let attempt=0;attempt<3;attempt++){ try{ res=await fetch("/api/ai-sales",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({user_id:userId,message}),signal:AbortSignal.timeout(50000)}); data=await res.json(); if(res.ok) break; }catch{ if(attempt===2) throw new Error(); } await new Promise(r=>setTimeout(r,1200*(attempt+1))); } setMessages(m=>[...m,{role:"bot",text:data.response||data.answer||data.message||"Готово. Уточните, пожалуйста, площадь комнаты."}]); } catch { setMessages(m=>[...m,{role:"bot",text:"Сейчас не удалось связаться с помощником. Попробуйте отправить сообщение ещё раз."}]); } finally {setLoading(false)} }
